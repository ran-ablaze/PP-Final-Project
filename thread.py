...
parser.add_argument("--thread", type=int, required=True, help="number of thread")
...
def discrete_predict(idx, task_per_thread, prior, test_images, test_labels, Q_prob_record, Q_error):
# def discrete_predict(idx, Q_error):
    error = 0
    
    wrong_record=[]
    prob_record=[]
    
    # start = int(idx*task_per_thread)
    # end = int((idx+1)*task_per_thread)

    for n in range (0, task_per_thread):
        # print("n ", n)
        # print("n:", n)
...
if __name__=='__main__':
    ...
    if args.mode==0: #DISCRETE
        ...
        Q_error = Queue()
        threads = []
        task_per_thread = (int) (10000/thread_num)
        for idx in range(thread_num):
            print("idx", idx)
            start = int(idx*task_per_thread)
            end = int((idx+1)*task_per_thread)
            t = threading.Thread(target=discrete_predict, args=(idx, task_per_thread, prior, test_images[start:end, :], test_labels[start:end], Q_prob_record, Q_error))
            # t = threading.Thread(target=discrete_predict, args=(idx, Q_error))
            t.start()
            threads.append(t)

        for thread in threads:
            print("wait join", thread)
            thread.join()
